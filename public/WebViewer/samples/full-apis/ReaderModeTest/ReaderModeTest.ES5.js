!(function(t) {
  ...
  function(t, n) {
    window.addEventListener('viewerLoaded', function() {
      var t = window.instance,
        n = window.parent.document;
      t.UI.enableElements(['readerPageTransitionButton']),
        (n.getElementById('select').onchange = function(n) {
          t.UI.loadDocument(n.target.value);
        }),
        (n.getElementById('file-picker').onchange = function(n) {
          var r = n.target.files[0];
          r && t.UI.loadDocument(r);
        }),
        (n.getElementById('url-form').onsubmit = function(r) {
          r.preventDefault(), t.UI.loadDocument(n.getElementById('url').value);
        }),
        t.Core.documentViewer.addEventListener(
          'documentLoaded',
          function() {
            t.UI.setZoomLevel(1), t.UI.toggleReaderMode();
          },
          { once: !0 }
        );
    });
